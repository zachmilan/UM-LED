...
extern osMutexId_t bm71LockHandle;
...
void ProcessTouch(void) {
	...
	if (delta_sum < TOUCH_THRESHOLD) { // no touch detected. reset haptics.
		...
		if (last_haptic_step == -1) {
	  	...
	  	osMutexAcquire(bm71LockHandle, osWaitForever); // wait JUST IN CASE a task is affecting internal bm71 state. (will likely never happen).
...
//		printf("[%d]:%d ", i, delta); // avoid printf in RTOS
	}
}
